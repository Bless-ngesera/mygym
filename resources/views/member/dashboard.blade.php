    ...
    <x-slot name="header">
        <div id="welcome-message" class="flex items-center justify-between">
            ...
            <div class="flex items-center gap-3 flex-wrap justify-end">
                <x-notification-bell :count="$unreadNotificationsCount ?? 0" :notifications="$recentNotifications ?? []" />
    ...
    <div class="py-12 bg-gray-50 min-h-screen relative"
        ...
        <div class="max-w-7xl mx-auto sm:px-6 lg:px-8 space-y-10 relative z-10">
            ...
            <div class="flex flex-wrap gap-3">
                <a href="{{ route('member.classes') }}" class="px-5 py-3 bg-gradient-to-r from-purple-600 to-indigo-600 text-white font-semibold rounded-full shadow hover:scale-[1.02] transform transition">
                    📅 Book a Class
                </a>
                <a href="{{ route('member.bookings') }}" class="px-5 py-3 bg-white text-gray-800 font-semibold rounded-full shadow border border-gray-100 hover:shadow-md transition">
                    📋 My Bookings
                </a>
                ...
                <div class="p-6 rounded-2xl bg-white/90 backdrop-blur-sm border border-yellow-100 shadow hover:shadow-lg transition transform hover:-translate-y-1">
                    <div class="flex items-start justify-between">
                        <div class="min-w-0 flex-1 pr-2">
                            ...
                            <p class="text-xl font-extrabold text-gray-900 mt-2 truncate">{{ $instructor->name ?? 'Assigned Soon' }}</p>
                            <p class="text-xs text-gray-500 mt-1">Personal Instructor</p>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-3 gap-6">
                ...
                <div class="lg:col-span-2 space-y-6">
                    ...
                    <div class="bg-white/90 backdrop-blur-sm shadow-lg rounded-2xl border border-gray-100 overflow-hidden">
                        ...
                        <div class="space-y-0">
                            @forelse($workoutFrequency->take(5) as $workout)
                                <div class="flex items-center gap-4 p-4 border-b border-gray-50 hover:bg-gray-50 transition">
                                    ...
                                    <div class="flex-1">
                                        <div class="flex items-center justify-between">
                                            <div>
                                                <p class="font-semibold text-gray-900">{{ $workout->date ? \Carbon\Carbon::parse($workout->date)->format('l, M d, Y') : 'Date' }}</p>
                    ...
                    <div class="bg-white/90 backdrop-blur-sm shadow-lg rounded-2xl border border-gray-100 overflow-hidden">
                        <div class="p-6 border-b border-gray-100">
                            <div class="flex items-center gap-3">
                                <div class="w-12 h-12 bg-gradient-to-br from-purple-600 to-indigo-600 rounded-full flex items-center justify-center text-white text-lg font-bold shadow-md">
                                    {{ strtoupper(substr($instructor->name, 0, 1)) }}
                                </div>
                                <div>
                                    <h3 class="text-lg font-bold text-gray-800">💬 Your Instructor</h3>
                        ...
                        <div class="p-6">
                            <div class="space-y-3 mb-4 max-h-48 overflow-y-auto" id="messagesContainer">
                                @forelse($recentMessages->take(5) as $message)
                                    <div class="flex {{ $message->sender_id === Auth::id() ? 'justify-end' : 'justify-start' }}">
                                        <div class="max-w-[80%] {{ $message->sender_id === Auth::id() ? 'bg-gradient-to-r from-purple-600 to-indigo-600 text-white' : 'bg-gray-100 text-gray-700' }} rounded-2xl px-4 py-2.5">
                                            <p class="text-sm">{{ $message->message }}</p>
                                            <p class="text-xs mt-1 opacity-60">{{ $message->created_at->diffForHumans() }}</p>
                                        </div>
                                    </div>
                                @empty
                                    <div class="text-center text-gray-500 py-4 text-sm">No messages yet. Start a conversation!</div>
                                @endforelse
                            </div>
                            <div class="flex gap-2">
                                <input type="text" id="messageInput" placeholder="Message your instructor..."
                                    class="flex-1 px-4 py-2 border border-gray-200 rounded-full focus:outline-none focus:ring-2 focus:ring-purple-500 text-sm transition-all"
                                    onkeypress="if(event.key==='Enter') sendMessage()">
                                <button onclick="sendMessage()"
                                    class="px-4 py-2 bg-gradient-to-r from-purple-600 to-indigo-600 text-white rounded-full text-sm font-semibold shadow hover:scale-[1.02] transform transition">
                                    Send
                                </button>
                            </div>
                        </div>
            ...
            <div class="grid grid-cols-1 lg:grid-cols-2 gap-6">
                <div class="bg-white/90 backdrop-blur-sm shadow-md rounded-2xl border border-gray-100 p-6">
                    <div class="flex items-center justify-between mb-4">
                        ...
                        <a href="{{ route('member.classes') }}" class="text-sm text-indigo-600 font-semibold hover:underline">See all</a>
                    </div>
                    <div class="space-y-4">
                        @php
                            $classList = [
                                ['title' => 'Pilates', 'schedule' => 'Mon & Wed, 6 PM', 'icon' => '🧘', 'color' => 'purple'],
                                ['title' => 'Yoga', 'schedule' => 'Tue & Thu, 7 AM', 'icon' => '🧘‍♀️', 'color' => 'emerald'],
                                ['title' => 'Dance Fitness', 'schedule' => 'Fri & Sat, 5 PM', 'icon' => '💃', 'color' => 'amber'],
                                ['title' => 'Boxing', 'schedule' => 'Mon & Fri, 7 PM', 'icon' => '🥊', 'color' => 'indigo'],
                            ];
                        @endphp
                        @foreach($classList as $class)
                            <div class="flex items-center gap-4 p-3 bg-gray-50 rounded-xl hover:bg-purple-50 transition">
                                ...
                                <a href="{{ route('member.classes') }}" class="text-sm text-indigo-600 font-semibold hover:underline">Join →</a>
                            </div>
                        @endforeach
                    </div>
                </div>
        ...
        <a href="{{ route('member.classes') }}" class="fixed bottom-8 right-8 bg-indigo-600 hover:bg-indigo-700 text-white p-4 rounded-full shadow-xl ring-4 ring-white/50 transform hover:scale-105 transition z-50" title="Book a Class">
