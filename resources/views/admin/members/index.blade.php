        ...
        <nav class="flex-1 overflow-y-auto px-3 space-y-0.5 pb-2 custom-scrollbar">
            ...
            <a href="{{ route('admin.members.index') }}"
               ...
               {{ request()->routeIs('admin.members.*') ? 'bg-indigo-600 text-white shadow-md' : 'bg-emerald-50 text-emerald-700' }}">
                ...
                @if($members->total() > 0)
                    <span class="ml-auto text-xs font-bold px-2 py-0.5 rounded-full {{ request()->routeIs('admin.members.*') ? 'bg-white/25 text-white' : 'bg-emerald-100 text-emerald-700' }}">
                        {{ $members->total() }}
                    </span>
                @endif
            </a>
                    ...
                    <div class="bg-white/85 backdrop-blur-md border border-white/40 rounded-2xl shadow-lg overflow-hidden">
                        ...
                        <div class="px-6 py-4 border-b border-gray-100 flex justify-between items-center">
                            ...
                            <div class="flex items-center gap-3">
                                ...
                                <div class="text-sm bg-emerald-50 text-emerald-700 px-3 py-1.5 rounded-lg font-medium">
                                    Total: <span class="font-bold">{{ $members->total() ?? 0 }}</span>
                                </div>
                            </div>
                        </div>
                                    ...
                                    @empty
                                        <tr>
                                            <td colspan="5" class="px-6 py-12 text-center">
                                                ...
                                                <a href="{{ route('admin.members.create') }}" class="inline-flex items-center gap-2 mt-3 text-emerald-600 hover:text-emerald-800 text-sm font-semibold">
                                                    <svg class="w-4 h-4" fill="none" stroke="currentColor" viewBox="0 0 24 24">
                            ...
                            <div class="px-6 py-4 border-t border-gray-100">
                                <div class="flex items-center justify-between">
                                    ...
                                    <div class="flex items-center gap-2">
                                        ...
                                        @else
                                            <a href="{{ $members->previousPageUrl() }}" class="px-3 py-1 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">&lt;</a>
                                        @endif

                                        {{-- Pagination Elements --}}
                                        @foreach($members->getUrlRange(1, $members->lastPage()) as $page => $url)
                                            @if($page == $members->currentPage())
                                                <span class="px-3 py-1 text-sm font-semibold text-indigo-600 bg-indigo-50 rounded-lg">{{ $page }}</span>
                                            @else
                                                <a href="{{ $url }}" class="px-3 py-1 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">{{ $page }}</a>
                                            @endif
                                        @endforeach

                                        {{-- Next Page Link --}}
                                        @if($members->hasMorePages())
                                            <a href="{{ $members->nextPageUrl() }}" class="px-3 py-1 text-sm text-gray-600 hover:text-indigo-600 hover:bg-indigo-50 rounded-lg transition-colors">&gt;</a>
                                        @else
                                            <span class="px-3 py-1 text-sm text-gray-400 cursor-not-allowed">&gt;</span>
                                        @endif
                                    </div>
