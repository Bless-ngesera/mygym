<nav x-data="{
                    ...
                    <div x-show="notificationOpen"
                        ...
                        <div class="p-3 border-b border-gray-100 dark:border-gray-700 flex justify-between items-center">
                            ...
                            <div class="flex gap-2">
                                <button x-show="unreadCount > 0"
                                        @click="fetch('/notifications/mark-all-read', { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }).then(() => { notificationOpen = false; location.reload(); })"
                                        class="text-xs text-purple-600 hover:text-purple-700 dark:text-purple-400 transition">
                                    Mark all read
                                </button>
                                <button @click="fetch('/notifications/clear-all', { method: 'DELETE', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }).then(() => { notificationOpen = false; location.reload(); })"
                        ...
                        <div class="max-h-80 sm:max-h-96 overflow-y-auto">
                            <template x-for="notification in notifications" :key="notification.id">
                                <div class="p-3 border-b border-gray-50 dark:border-gray-700 hover:bg-gray-50 dark:hover:bg-gray-700/50 transition"
                                     :class="{ 'bg-purple-50/30 dark:bg-purple-900/20': !notification.read }">
                                    <div class="flex items-start gap-3">
                                        <div class="w-8 h-8 rounded-full flex items-center justify-center flex-shrink-0"
                                             :class="{
                                                 'bg-blue-100 text-blue-600': notification.type === 'booking',
                                                 'bg-yellow-100 text-yellow-600': notification.type === 'achievement',
                                                 'bg-purple-100 text-purple-600': notification.type === 'info',
                                        ...
                                        <div class="flex-1 min-w-0">
                                            ...
                                            <p class="text-xs text-gray-500 dark:text-gray-400 mt-0.5" x-text="notification.message"></p>
                                            <p class="text-xs text-gray-400 dark:text-gray-500 mt-1" x-text="notification.created_at"></p>
                                        </div>
                                        <button x-show="!notification.read"
                                                @click="fetch(`/notifications/${notification.id}/read`, { method: 'POST', headers: { 'X-CSRF-TOKEN': '{{ csrf_token() }}' } }).then(() => { notification.read = true; unreadCount--; })"
    ...
    <div :class="{'block': open, 'hidden': ! open}" class="hidden md:hidden bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border-t border-gray-100 dark:border-gray-800">
        <div class="pt-2 pb-3 space-y-1 px-4">
            @auth
                <!-- Mobile AI Assistant Button -->
                <div class="px-3 py-2 mb-2">
                    <button onclick="openChat()" class="w-full bg-gradient-to-r from-purple-600 to-blue-600 hover:shadow-lg transition-all duration-300 rounded-xl px-4 py-3 flex items-center justify-center gap-2">
                ...
                <div class="px-3 py-2 border-b border-gray-100 dark:border-gray-800 mb-2">
                    ...
                    <div class="flex gap-2">
                        <button @click="theme = 'light'" class="flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all" :class="theme === 'light' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'">
                            Light
                        </button>
                        <button @click="theme = 'dark'" class="flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all" :class="theme === 'dark' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'">
                            Dark
                        </button>
                        <button @click="theme = 'system'" class="flex-1 px-3 py-2 rounded-lg text-sm font-medium transition-all" :class="theme === 'system' ? 'bg-purple-100 dark:bg-purple-900/30 text-purple-700 dark:text-purple-400' : 'bg-gray-100 dark:bg-gray-800 text-gray-700 dark:text-gray-300'">
                            System
                        </button>
                    </div>
                </div>
